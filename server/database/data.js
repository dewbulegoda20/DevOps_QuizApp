export default [
    {
        id: 1,
        question : "Javascript is an _______________ language",
        options : [
            'Object-Oriented',
            'Object-Based',
            'Procedural',
        ]
    },
    {
        id: 2,
        question : "Following methods can be used to display data in some form using JavaScript",
        options : [
            'document.write()',
            'console.log()',
            'window.alert()',
        ]
    },
    {
        id: 3,
        question : "When an operator value is NULL, the type of returned by the unary operator is:",
        options : [
            'Boolean',
            'Undefined',
            'Object',
        ]
    },
    {
        id: 4,
        question : "What does the toString() method return?",
        options : [
            'Return Object',
            'Return String',
            'Return Integer',
        ]
    },
    {
        id: 5,
        question : "Which fuction is used to serialize an object into a JSON string",
        options : [
            'stringfy()',
            'parse()',
            'convert()',
        ]
    }

];

export const answers = [0, 1, 2, 1, 0];

